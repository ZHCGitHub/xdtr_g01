 ...
 * Date:2017/4/17
 * Time:14:10
 * 功能：根据广度优先获取网站的所有url。并且在每次向下层爬取之前，先把本层的源码爬下来，生成文件和md5码。
 * 下次在爬取的时候生成文件和md5码与上次结果比较，如果md5码不同，则向下爬取，否则略过。
 * 参考网站：
 */
public class WebCrawlerBreadthFirst {

    Map mySpider(String baseUrl, String charset, String path, Connection conn) {
        // 创建一个map，存储链接-是否被遍历
        ...
        oldMap = chackMd5(oldLinkHost, oldMap, charset, path, conn);
        return oldMap;
    }
    ...
    private Map chackMd5(String oldLinkHost, Map<String, Boolean> oldMap
            , String charset, String path, Connection conn) {
                ...
                if (file_old.exists() && md5File_old.exists()) {
                    ...
                    try {
                        ...
                        if (Objects.equals(md5_old, md5_new) || md5_old.equals(md5_new)) {
                            ...
                            //向mysql中插入网站对应的变化率
                            String sql = "REPLACE INTO tbc_md_url_percentage VALUES(\"" + url + "\"," + 0 + ")";
                            MysqlConnectUtil.insert(conn, sql);
                            ...
                            //算出新旧源码文件之间的差异率
                            float web_percentage = Float.parseFloat(compareFile.compare(conn, url, filePath_old, filePath_new));
                            ...
                            //md5校验不一致
                            return catchLinks(oldLinkHost, oldMap, charset);
                        }
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    ...
                    if (!file_old.exists()) {
                        ...
                        //向mysql中插入网站对应的变化率
                        String sql2 = "REPLACE INTO tbc_md_url_percentage VALUES(\"" + oldLinkHost.replaceAll("http://", "") + "\"," + 0 + ")";
...
//                    e.printStackTrace();
                    //三次失败重试
                    try {
                        System.out.println("================" + oldLink + "第一次失败重试====================");
                        newMap = crawlError(oldLinkHost, oldMap, oldLink, charset);
                    } catch (Exception e1) {
                        try {
                            System.out.println("================" + oldLink + "第二次失败重试====================");
                            newMap = crawlError(oldLinkHost, oldMap, oldLink, charset);
                        } catch (Exception e2) {
                            try {
                                System.out.println("================" + oldLink + "第三次失败重试====================");
                                newMap = crawlError(oldLinkHost, oldMap, oldLink, charset);
                            } catch (Exception e3) {
                                e3.printStackTrace();
                            }
//
        ...
        if (!newMap.isEmpty()) {
            ...
            oldMap.putAll(catchLinks(oldLinkHost, oldMap, charset));  //由于Map的特性，不会导致出现重复的键值对
        }
        return oldMap;
    }
...
//                                System.out.println("Link:"+newLink);
    ...
    public static void main(String[] args) {
        WebCrawlerBreadthFirst webCrawlerDepthFirst = new WebCrawlerBreadthFirst();
        //设置mysql连接的参数
        ...
        Map<String, Boolean> map = webCrawlerDepthFirst.mySpider("http://10.0.0.2",
                "gb2312", "E:\\g01_write\\", conn);
        //https://www.tmall.com                 utf-8
        //https://tieba.baidu.com/index.html    utf-8

        //http://www.zmdgtj.gov.cn      gb2312
        //http://www.zol.com.cn         gbk
